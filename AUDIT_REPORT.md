...
| E1 | Ekonomi | High | Pratik yarış/PvP: cüzdan güncellemesi ile yarış kaydı iki AYRI transaction'da — arada hata olursa çift ödeme/kayıp riski *(bu turda tekrar doğrulandı — hâlâ AÇIK, bkz. `run-practice-race.use-case.ts`)* |
...
| T2 | Frontend | Medium | ⚠️ **KISMEN DÜZELTİLDİ** — `api-client.ts` için kontrat testleri eklendi; `.tsx` component testleri (jsdom eksikliği) hâlâ AÇIK |
...
**⚠️ KISMEN TAMAMLANDI (bu turda doğrulandı):** `api-client.ts` için gerçek kontrat testleri eklendi (commit `6075da9`). `.tsx` component testleri (ör. `RaceHud.tsx`, `player-context.tsx`) `jsdom` devDependency eksikliği yüzünden HÂLÂ AÇIK — sıradaki adım olarak planlandı.
...
8. ✅ (kısmen) **T1+T2+T3** — T1 (eşzamanlılık) **TAMAMLANDI VE CI'DA DOĞRULANDI**; T2 kısmen (`api-client.ts` kontrat testleri var, `.tsx` component testleri `jsdom` eksikliği yüzünden AÇIK); T3 (100+ yarış denge testi) HÂLÂ AÇIK.

**Durum özeti (2026-09-19 itibarıyla):** Madde 1-7'nin TAMAMI ve madde 8'in T1 kısmı kapatıldı, gerçek CI'da doğrulandı (bkz. `claude/hizli-bitirme-plani.md` proje dokümanındaki detaylı kronoloji). G1 ve DOC1 (Low severity, ayrı bölümlerde) de bu turda kapatıldı. **Hâlâ AÇIK olan gerçek bulgular:** E1 (pratik yarış/PvP'de cüzdan+yarış kaydının iki ayrı transaction'da olması — bu turda tekrar doğrulandı), T2'nin `.tsx` component test kısmı, T3 (100+ yarış denge/adalet testi), F1 (mobil responsive), F2 (WebSocket, bilinçli PLANNED), R2 (tam alan replay), R3 (davranış hattının yarısı hâlâ placeholder).
