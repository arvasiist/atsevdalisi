...
| T3b | Denge/Tasarım | Low | 🆕 **YENİ BULGU** — "closer" taktiği eşit statlı bir alanda ~%53 galibiyet payı alıyor (beklenen %25), motorun `pace` config'indeki asimetriden kaynaklanıyor; rebalancing proje sahibinin kararını gerektirdiği için kapsam dışı bırakıldı, test ile izleniyor |
...
### T3b — Low (YENİ BULGU, bu turda T3 testi tarafından keşfedildi): `closer` taktiği istatistiksel olarak özdeş bir alanda orantısız galibiyet payı alıyor
**Evidence:** `race-engine-field-balance.spec.ts`'in CI #114'teki İLK çalıştırması: 12 atlık, tamamen özdeş statlı bir alanda (yalnızca `racingStyle` farklı) 250 denemenin **%52.8'ini (132/250) "closer" kazandı** — 4 eşit temsil edilen stil için "taraf tutmayan" bir motorda beklenen pay %25'in iki katından fazla. Kök neden `domain/race/pace.ts`'teki `derivePaceEffect` + `race.config.json`'ın `pace` bölümü: `closer` TÜM yarış boyunca %15 daha AZ stamina tüketir (`closerStaminaMultiplier: 0.85`) VE SON düzlükte (`finalStretchMeters: 400`, 1600m'de son 2/8 segment) AYRICA +4 performans bonusu alır — yani hem daha az yorulur HEM DE tam da en kritik anda ekstra bonus kazanır. `front_runner` TERS yönde asimetriktir: TÜM yarış boyunca %15 DAHA FAZLA stamina tüketir (`frontRunnerStaminaMultiplier: 1.15`) ama +3 bonusu yalnızca SON düzlük DIŞINDAKİ segmentlerde alır — cezası her zaman işler, ödülü yarışın en kritik anında (bitişte) kesilir.
**Impact:** Oyun tasarımı açısından: rasyonel bir oyuncu için "closer" diğer taktiklere göre BELİRGİN ŞEKİLDE üstün bir seçim haline gelir (eşit statlarda ~2x kazanma şansı) — bu, brief'in taktik seçiminin ANLAMLI bir karar olması gerektiği ilkesiyle (§14.2, §18) gerilim yaratır. Acil bir güvenlik/ekonomi riski DEĞİLDİR (para/istismar değil, oyun dengesi).
**Fix:** `race.config.json`'ın `pace` bölümündeki asimetriyi düzeltmek — ör. `closerStaminaMultiplier`'ı yukarı çekmek, `closerLateStageBonus`'u azaltmak, veya `frontRunnerPositionBonus`'un da SON düzlükte geçerli olmasını sağlamak. Bu GERÇEK bir oyun dengesi/tasarım kararı olduğundan (tüm pratik yarışları ve PvP'yi etkiler) bu turun kapsamı dışında bırakıldı — proje sahibinin onayı/tercihi gerekir.
**Test requirement:** `race-engine-field-balance.spec.ts` bu dengesizliği ARTIK İZLİYOR (regresyon eşiği %58) — `pace` config'i yeniden dengelenirse bu testin eşikleri de gerçek yeni ölçüme göre SIKILAŞTIRILMALI (gevşetilmiş eşik kalıcı bir onay değil, geçici bir belgeleme).
...
8. ✅ **T1+T2+T3** — T1 (eşzamanlılık) **TAMAMLANDI VE CI'DA DOĞRULANDI**; T2 **TAMAMLANDI** (`api-client.ts` kontrat testleri + `RaceHud`/`player-context` component testleri, `jsdom` eklendi); T3 **TAMAMLANDI VE CI'DA DOĞRULANDI** (12 atlık alan testi — bkz. T3b, bu testin keşfettiği yeni bir denge bulgusu).
9. ✅ **E1** — Pratik yarış cüzdan+yarış kaydı atomikliği. **DÜZELTİLDİ VE GERÇEK CI'DA DOĞRULANDI** (commit `a010ec3`, CI #111).

**Durum özeti (2026-09-19 itibarıyla):** Madde 1-9'un TAMAMI (T1+T2+T3 dahil) kapatıldı, gerçek CI'da doğrulandı (bkz. `claude/hizli-bitirme-plani.md` proje dokümanındaki detaylı kronoloji). G1 ve DOC1 (Low severity, ayrı bölümlerde) de bu turda kapatıldı. T3 testi bir YAN ÜRÜN olarak yeni bir bulgu keşfetti (T3b — bkz. yukarısı). **Hâlâ AÇIK olan gerçek bulgular:** T3b ("closer" taktik dengesizliği — proje sahibinin tasarım kararını gerektirir), F1 (mobil responsive), F2 (WebSocket, bilinçli PLANNED), R2 (tam alan replay), R3 (davranış hattının yarısı hâlâ placeholder), ve PvP'nin E1 ile aynı yapısal desendeki (ama mali riski olmayan) transaction ayrımı.
