...
**✅ KISMEN DÜZELTİLDİ (proje sahibinin Draw'dan SONRA "sen karar ver, devam et" yetkilendirmesiyle seçilen üçüncü alt-dilim): Track Fit artık gerçek.** Altı alandan (Track Fit, Draw, Carried Weight, Current Form, Jockey skill, Temperament) Track Fit, Draw ile AYNI "şema zaten var, tüketen formül zaten var, eksik olan yalnızca wiring" kategorisindeydi: `horse_surface_stats`/`horse_distance_stats` (migration 0003) ve `base-ability.ts`'in `trackCompatibility = (surfaceCompatibility + distanceCompatibility) / 2` formülü ZATEN vardı — Carried Weight (formülde hiç yer YOK, yeni bir denge terimi icat etmeyi gerektirirdi, T3b riski) ve Temperament/Jockey skill (gerçek bir jokey-atama akışı gerektirir, `race_entries.jockey_id` HER ZAMAN `NULL`) bu yüzden yine bilinçli olarak dışarıda bırakıldı.

Uygulama: `PostgresHorseRepository.save()` artık `horse_stats`/`horse_health` ile AYNI desende `horse_surface_stats`/`horse_distance_stats`'a da varsayılan (DB DEFAULT 50) birer satır ekliyor; yeni migration `0026_wire_horse_surface_and_distance_stats` bu değişiklikten ÖNCE oluşturulmuş atları da backfill ediyor VE migration 0022'nin artık geçersiz "TASARLANDI ama HENÜZ WIRING EDİLMEDİ" `COMMENT ON TABLE` notunu günceller. Yeni salt-okunur `HorseSurfaceStatsRepository`/`HorseDistanceStatsRepository` portları + `PostgresHorseSurfaceStatsRepository`/`PostgresHorseDistanceStatsRepository` (scout/keşif mekaniği brief §34 BİLİNÇLİ olarak kapsam dışı bırakıldığından `update` metodu YOK). Yeni saf domain dosyası `domain/race/track-fit.ts`: `computeSurfaceCompatibility`/`computeDistanceCompatibility` — zemin türü (grass/dirt) doğrudan eşlenir, `'synthetic'` (bugün codebase'de HİÇBİR gerçek yarış yolunda kullanılmıyor) için nötr 50 döner; mesafe, gerçek at yarışçılığı geleneğiyle uyumlu sabit eşiklerle (< 1400m Short, 1400-2000m Middle, > 2000m Long — projedeki TEK gerçek mesafe olan 1600m "Middle"ya düşüyor) kategorize edilir. `wet`/`heavy`/`dry`/`mud` (pist koşulu) sütunları BİLİNÇLİ olarak kullanılmadı — bu, `environment.ts`'in `getEnvironmentModifier`'ının ZATEN ayrı bir mekanizma olarak uyguladığı bir etkidir, ikisini tek bir sayıda birleştirmek YENİ bir denge kararı olurdu. `buildHorseEntrantSnapshot` yeni OPSİYONEL bir `trackFit` parametresi aldı (`recentResults` ile AYNI "verilmezse nötr kalır, GERİYE DÖNÜK uyumlu" deseni); `RunPracticeRaceUseCase` ve `JoinMatchmakingQueueUseCase.playMatch` bu iki sorguyu (salt okunur, mevcut `Promise.all`'a eklendi) çağırıp gerçek veriyi geçiriyor. `surfaceCompatibility`/`distanceCompatibility` artık `UNMODELED_SNAPSHOT_FIELDS` listesinde DEĞİL (tripwire testi güncellendi, liste artık yalnızca `jockeySkillComposite` içeriyor) — `form`'un daha önce aynı listeden çıkarılmasıyla AYNI desen.

Test: yeni `track-fit.spec.ts` (saf domain, NestJS/DB bağımsız) her iki fonksiyonun sınır davranışını (zemin eşlemesi, `'synthetic'` nötr fallback, mesafe eşiklerinin DAHİL/HARİÇ ayrımı) test eder; `entrant-snapshot.spec.ts`'e `trackFit` verilen/verilmeyen/`null` verilen senaryolar + güncellenmiş [TRIPWIRE] testi eklendi. Push ÖNCESİ, `computeSurfaceCompatibility`/`computeDistanceCompatibility` framework-bağımsız SAF fonksiyonlar olduğundan bu sandbox'ta `apps/api/tsconfig.domain.json` ile GERÇEK `tsc --noEmit` (0 hata) VE `tsx` ile GERÇEK KOD çalıştırması (10/10 assertion başarılı) ile doğrulandı — Draw/Current Form'un `tsx`-only doğrulamasından bir adım daha güçlü, çünkü `domain/race/**` TAMAMEN bu sandbox'ta tip-denetlenebiliyor (CI #134'ün `readonly`→mutable dersi göz önünde bulundurularak `TrackFitInput`'ın `surfaceStats`/`distanceStats` alanları BİLEREK mutable bırakıldı, hiçbir yerde `readonly` bir diziyi mutable bir tipe atama YOK). NestJS/pg'ye bağımlı dosyalar (yeni repository'ler, iki use-case) bu sandbox'ta kurulu `node_modules` olmadığından yalnızca `ts.transpileModule` ile sözdizimi düzeyinde doğrulanabildi (Draw/Current Form/F2 ile AYNI sınır) — gerçek tip doğrulaması push+CI'da.

**Sonuç — commit `8b93b04`, CI #141'de TAM YEŞİL doğrulandı (2m 41s, migration 0026 dahil `Run database migrations` adımı sorunsuz geçti, 11 uyarı + 1 bildirim — öncekiyle AYNI temel seviye, 0 hata; push+kontrol bağımsız built-in tarayıcı ile yapıldı).** R3'ün Track Fit alt-dilimi artık TAMAMEN kapandı — R3'ün altı alanından ÜÇÜ (Current Form, Draw, Track Fit) gerçek veriye bağlandı.
...
**GÜNCELLENDİ (2026-09-24, proje sahibinin "Sen karar ver, devam et" yetkilendirmesiyle) — Görsel Kalite Faz 4'ün İLK DİLİMİ eklendi:** Yukarıdaki Bloom/SSAO/Environment/2048px gölgeler ARTIK HER cihazda AYNI ağırlıkta çalışmıyor — yeni `apps/web/src/features/race-viewer/quality-tier.ts` (saf, bu sandbox'ta GERÇEKTEN `tsc --noEmit` + `tsx` ile 31/31 assertion doğrulandı) `navigator.userAgent`/`hardwareConcurrency`'den Master Plan §46'nın dört kademesini ('low'/'medium'/'high'/'ultra') türetir, `RaceScene3D.tsx` bu kademeye göre Environment/Bloom/SSAO/gölgeleri/piksel oranını AÇIP KAPATIR. Yüksek çekirdekli bir masaüstünde ('ultra') ayarlar bu değişiklikten ÖNCEKİ sabit değerlerle BİREBİR AYNI — görsel regresyon YOK, yalnızca düşük donanımlı/mobil cihazlar için bir kaçış yolu eklendi. **Bilinçli olarak KAPSAM DIŞI bırakılan** (§46'nın geri kalanı): LOD/texture compression (gerçek 3D model/doku henüz YOK, Faz 3'ün asset kararını bekliyor), animation pooling (animasyon sistemi henüz YOK), gerçek 4/8/12/16 at FPS benchmark'ı (bu sandbox'ta gerçek tarayıcı/GPU yok, ölçülemez — proje sahibinin manuel QA'sı gerekiyor). Detaylı gerekçe `quality-tier.ts` dosya başı doc yorumunda ve `race-viewer/README.md`'de. Yerel doğrulama sınırı AYNI (JSX/three.js içeren `RaceScene3D.tsx`'in kendisi yalnızca `ts.transpileModule` ile sözdizimi kontrolünden geçirilebildi, gerçek doğrulama CI'dadır).
