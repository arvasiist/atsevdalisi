...
| R3 | Yarış Motoru | Low | ✅ **KISMEN DÜZELTİLDİ** (commit `f65db23`, CI #125 — Current Form; yeni commit, CI doğrulaması bekleniyor — Draw/post-position, bkz. `gate-assignment.ts`) — Current Form artık `race_entries` geçmişinden gerçekten türetiliyor (`deriveFormFromRecentResults`); Draw artık gerçek bir çekilişten türetiliyor (`assignGatePositions`, `race_entries.gate_position` ilk kez dolduruluyor, YARIŞ SONUCUNU etkilemiyor); Track Fit/Carried Weight/Temperament/gerçek Jockey (yeni tablo/sistem gerektirdiklerinden) hâlâ nötr placeholder, bilinçli olarak Master Plan Phase B/C'ye bırakıldı |
...
**CI #128 kırmızı çıktı, commit `<gate-assignment-fix>` ile düzeltildi — kök neden `race-engine.ts`/`bot-generator.ts` gibi bu projedeki HİÇBİR dosyada daha önce karşılaşılmamış YENİ bir hata sınıfıydı (`RaceGateway`/`realtime.e2e-spec.ts`'in CI #126'daki İKİ test-only hatasından FARKLI olarak, bu kez GERÇEK `gate-assignment.ts` kaynak kodundaydı):** Proje `tsconfig.base.json`'da `noUncheckedIndexedAccess: true` kullanıyor — yani bir dizinin `[i]` ile okunması TypeScript'e göre HER ZAMAN `T | undefined` döner (döngü sınırları çalışma zamanında ne kadar garanti olursa olsun, derleyici bunu statik olarak bilemez). `assignGatePositions`'ın Fisher-Yates takas satırı (`[shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]]`) bu yüzden `Typecheck` adımında `Type 'string | undefined' is not assignable to type 'string'` hatasıyla derlenmiyordu — bu sandbox'ta `apps/api`'nin `node_modules`'ı/gerçek `tsc -p tsconfig.json`'ı hiç kurulup çalıştırılamadığından (npm registry erişimi yok, oturum boyunca geçerli AYNI kısıt), push ÖNCESİ yapılan `tsx`-tabanlı doğrulama (saf ESM/CJS runtime çalıştırması) bu STATİK tip hatasını YAKALAYAMAMIŞTI — yalnızca push+CI round-trip'i gerçek `tsc`'yi çalıştırdığından hatayı ortaya çıkardı. **Düzeltme:** `!` ile tip zorlaması YAPILMADI — bunun yerine dosya içi yeni bir `readIndexOrThrow` yardımcı fonksiyonu, okunan değerin GERÇEKTEN `undefined` olup olmadığını ÇALIŞMA ZAMANINDA kontrol edip (döngü matematiği bir gün bozulursa sessizce `undefined` yazmak yerine AÇIKÇA `Error` fırlatarak) `string` tipini GERÇEKTEN garanti ediyor — bkz. `gate-assignment.ts`'in güncellenmiş doc yorumu. Düzeltme sonrası `gate-assignment.spec.ts`'in tüm senaryoları (+ yeni bir 20-katılımcılı büyük-N testi) yeniden `tsx` ile çalıştırılıp davranışın DEĞİŞMEDİĞİ (aynı permütasyonlar, aynı determinizm) doğrulandı.
...
**Bilinçli kapsam DIŞI (gelecek dilimler):** (1) Senkronize çoklu-izleyici odaları — her istemci KENDİ abone olma anına göre bağımsız bir replay alır, aynı yarışı izleyen iki izleyici birbiriyle senkronize DEĞİLDİR. (2) `notification.new`/`lobby.update` (docs/API.md §10'un diğer iki önerisi) bu dilimde YOK. (3) Yeniden bağlanma/kaldığı yerden devam etme YOK (replay idempotent olduğundan istemci `race.subscribe`'ı baştan çağırabilir, zarar vermez).
