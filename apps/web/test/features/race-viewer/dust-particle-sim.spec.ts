...
const config = vfxConfigJson as unknown as VfxConfig;

describe('spawnDustParticle', () => {
  it('doğum anında konum origin ile aynıdır', () => {
    const particle = spawnDustParticle(10, 20, 'h1:0', config);
  ...
  it('aynı seed her zaman aynı lifetimeMs\'i üretir (determinizm)', () => {
    const a = spawnDustParticle(0, 0, 'h1:5', config);
    const b = spawnDustParticle(0, 0, 'h1:5', config);
    expect(a).toEqual(b);
  });

  it('farklı seed farklı bir lifetimeMs üretebilir', () => {
    const a = spawnDustParticle(0, 0, 'h1:0', config);
    const b = spawnDustParticle(0, 0, 'h1:1', config);
  ...
  it('lifetimeMs her zaman [minLifetimeMs, maxLifetimeMs) aralığındadır', () => {
    for (let i = 0; i < 20; i += 1) {
      const particle = spawnDustParticle(0, 0, `seed-${i}`, config);
      expect(particle.lifetimeMs).toBeGreaterThanOrEqual(config.dustParticles.minLifetimeMs);
      expect(particle.lifetimeMs).toBeLessThan(config.dustParticles.maxLifetimeMs);
    }
  });
});
...
describe('advanceDustParticle', () => {
  it('yaşı deltaMs kadar ilerletir', () => {
    const particle = spawnDustParticle(0, 0, 'h1:0', config);
    const advanced = advanceDustParticle(particle, 100, 'h1:0', config);
    expect(advanced.ageMs).toBe(100);
  });

  it('yükseklik zamanla artar', () => {
    const particle = spawnDustParticle(0, 0, 'h1:0', config);
    const advanced = advanceDustParticle(particle, 500, 'h1:0', config);
    expect(advanced.y).toBeCloseTo(0.5 * config.dustParticles.riseSpeedMps, 6); // 0.5s * riseSpeedMps
  });

  it('aynı seed ile tekrar tekrar ilerletildiğinde yatay konum origin\'den bir yöne doğru KAYAR (sabit kalmaz)', () => {
    const particle = spawnDustParticle(5, 5, 'h1:0', config);
    const advanced = advanceDustParticle(particle, particle.lifetimeMs, 'h1:0', config);
  ...
  it('farklı seed ile ilerletmek FARKLI bir yatay konum üretebilir (yön seed\'e bağlıdır)', () => {
    const particle = spawnDustParticle(5, 5, 'h1:0', config);
    const advancedA = advanceDustParticle(particle, particle.lifetimeMs, 'h1:0', config);
    const advancedB = advanceDustParticle(particle, particle.lifetimeMs, 'h1:1', config);
    const differs = advancedA.x !== advancedB.x || advancedA.z !== advancedB.z;
    expect(differs).toBe(true);
  });

  it('mevcut alanları (originX/originZ/lifetimeMs) korur', () => {
    const particle = spawnDustParticle(3, 4, 'h1:0', config);
    const advanced = advanceDustParticle(particle, 50, 'h1:0', config);
...
describe('isDustParticleExpired', () => {
  ...
  it('yaş lifetime\'a ulaştıysa veya geçtiyse true döner', () => {
    const particle = spawnDustParticle(0, 0, 'h1:0', config);
    const expired = advanceDustParticle(particle, particle.lifetimeMs, 'h1:0', config);
    expect(isDustParticleExpired(expired)).toBe(true);
    const wayExpired = advanceDustParticle(particle, particle.lifetimeMs + 1000, 'h1:0', config);
...
describe('getDustParticleOpacity', () => {
  ...
  it('yarı ömründe yaklaşık 0.5 döner', () => {
    const particle = spawnDustParticle(0, 0, 'h1:0', config);
    const halfway = advanceDustParticle(particle, particle.lifetimeMs / 2, 'h1:0', config);
    expect(getDustParticleOpacity(halfway)).toBeCloseTo(0.5, 6);
  });

  it('ömrü aşan bir yaş için negatif olmayan bir değer döner (clamp)', () => {
    const particle = spawnDustParticle(0, 0, 'h1:0', config);
    const wayExpired = advanceDustParticle(particle, particle.lifetimeMs * 3, 'h1:0', config);
